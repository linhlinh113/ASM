...
@WebServlet(urlPatterns = {"", "/HomeServlet"}) // Chạy khi truy cập root (/)
public class HomeServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            // 5. Đẩy tất cả dữ liệu vào request
            req.setAttribute("categoryList", categoryList);
            req.setAttribute("hotList", hotList);
            req.setAttribute("newestList", newestList);
            req.setAttribute("homeNews", homeNews);
            
            // 6. Chuyển sang index.jsp để hiển thị
            req.getRequestDispatcher("/index.jsp").forward(req, resp);

        } catch (Exception e) {
            e.printStackTrace();
            // Hiển thị lỗi ra trang chủ
            req.setAttribute("error", "Không thể tải dữ liệu trang chủ: " + e.getMessage());
            req.getRequestDispatcher("/index.jsp").forward(req, resp);
        }
    }
}